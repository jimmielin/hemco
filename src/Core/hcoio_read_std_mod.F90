...
!!!             CALL CheckMissVal ( Lct, ncArr2 )
 ...
 110      FORMAT( 'Field ', a, ': Average data over years ', I4.4, ' to ', I4.4 )
    ...
    IF ( nlev > 1 ) THEN
      ...
      ! Debug message
      WRITE(MSG,*) '  ==> WRF/CESM: Always forcing MESSy regridding for number of verticals', nlev
      CALL HCO_MSG(HcoState%Config%Err,MSG)
    ENDIF
#endif
    ...
    IF ( UseMESSy ) THEN
       ...
       IF ( nlev > 1 .AND. .NOT. IsModelLevel ) THEN
          ...
          ! ...except in WRF and CESM, where vertical grids are arbitrary. (hplin, 5/27/20)
       ...
       ENDIF ! nlev>1 .and. not. IsModelLevel
